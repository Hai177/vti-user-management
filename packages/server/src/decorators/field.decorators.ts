...
import { IsNullable, IsPassword, IsPhoneNumber, IsUndefinable } from './validator.decorators';
...
type ClassValidateOptions = {
  validate?: Parameters<typeof Validate>;
};
...
export function ClassField<TClass extends Constructor>(
  getClass: () => TClass,
  options: Omit<ApiPropertyOptions, 'type'> & IClassFieldOptions & ClassValidateOptions = {}
): PropertyDecorator {
  ...
  if (options.validate) {
    decorators.push(Validate(...options.validate));
  }
...
export function ClassFieldOptional<TClass extends Constructor>(
  getClass: () => TClass,
  options: Omit<ApiPropertyOptions, 'type' | 'required'> & IClassFieldOptions & ClassValidateOptions = {}
): PropertyDecorator {
